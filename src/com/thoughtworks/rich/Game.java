...
public class Game {
    ...
    public void setMap(Map map) {
        ...
        for(int i=0;i<map.getSize();i++){
            if(map.getLand(i).getType() == Land.Type.hospital){
    ...
    public void playerHitOnBomb(Player player) {
        player.beInHospital(ToolBomb.getHospitalDay(), getHospitalLocation());
    }

    public Land getLandWithToolOnFrontofPlay(Player player, int numMove) {
        int location = player.getLocation();
        for (int i=0;i<=numMove;i++){
            int locationMove = (location + i)%getMap().getSize();
            Land land = getMap().getLand(locationMove);
...
//        String[] stringsMap = getTotalMapStringLine();
    ...
    public String[] getTotalMapStringLine() {
        ...
        stringsTotal[i] = getMap().toString();
        return stringsTotal;
    ...
    public String[] getPlayersAndToolsString(){
        ...
        for (int i=0;i<getMap().getSize();i++){
            Tool tool = getMap().getLand(i).getTool();
            if(tool != null){
    ...
    public String getLandOwnerString(){
        ...
        for(int i=0;i<getMap().getSize();i++){
            ...
            stringBuffer.append(getMap().getLand(i).getOwnerMark());
        }
    ...
    public void currentLandActionBy(Player player) {
        Land land = getMap().getLand(player.getLocation());
        land.action(player);
    }
    ...
    public void playerUseRoadBlockAndBomb(Player player, Tool.Type type, int offset) {
        ...
        int location = ((player.getLocation() + offset) + getMap().getSize())%getMap().getSize();
        Land land = getMap().getLand(location);
