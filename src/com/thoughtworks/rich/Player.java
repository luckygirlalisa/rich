 ...
 */
public class Player {
    enum DisableState {normal,hospital,prison}
    enum GodState {none,mascot}
    ...
    protected int money;
    protected int point;
    protected LinkedList<Land> lands = new LinkedList<Land>();
    ...
    public void buyLand(Land land) {
        money -= ((LandSpace)land).getValue();
        addLand(land);
        ((LandSpace)land).setOwner(this);
    }

    public void payForTravelling(LandSpace landSpace) {
        if(!willPayForTravelling(landSpace)){
            return;
        }
        int TravellingExpense = landSpace.getTravellingExpense();
        money -= TravellingExpense;
        Player landSpaceOwner = landSpace.getOwner();
    ...
    public boolean willPayForTravelling(LandSpace landSpace) {
        if(getGodState() == GodState.mascot){
            InputOutput.outputStringLine("福神附体，免过路费。");
            return false;
        }
        Player player = landSpace.getOwner();
    ...
    public void update(LandSpace landSpace) {
        money -= landSpace.getValue();
        landSpace.setLevel(landSpace.getLevel()+1);
    }
    ...
    public int getLandNumWithLvl(int level) {
        ...
        for (int i=0;i<getLandNum();i++){
            if(level == ((LandSpace)lands.get(i)).getLevel()){
                count++;
            }
        }
        return count;
    ...
    public boolean sellLand(Land land) {
        if(lands.remove(land)){
            ((LandSpace)land).setOwner(null);
            setMoney(getMoney() + 2 * ((LandSpace) land).getCurrentValue());
            return true;
        }
        return false;
    }
